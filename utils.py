...
def load_graph(adj_file):
    ...
    adj = sp.coo_matrix((np.ones( ddi_arr.shape[0]), ( ddi_arr[:, 0], ddi_arr[:, 1])),
                        shape=(832, 832), dtype=np.float32)
...
class load_data(Dataset):
    def __init__(self, dataset):
        ...
        #DF=pd.read_csv('data/z_class572.csv', dtype=float, header=None, index_col=0)
        #DF=DF.values
        #print("STE.shape",DF.shape)
        #self.x=DF

        self.x = np.loadtxt('./data/con.txt', dtype=float)
###when test different ddi class dateset,need to introduce  different file "ddi_class_X.csv"
        y = pd.read_csv('./graph/ddi_class_65.csv', dtype=int)
