...
class AE(nn.Module): ##Aotoencoder one drug's features outputting the layers of deep learning and the final layer
...
def train_dmddi(dataset):
    ...
    if epoch == 4:
        emb_all= np.array(emb.detach().numpy())    #change tensor  to array（832，65）
...
def combine_and_predict(embedding_look_up, ddi_adj, clf_type, event_num, seed, CV, label):
    ...
    for k in range(CV):  # CV=5
        ...
        if clf_type == 'DDIMDL':
            ...
            dnn.fit(x_train, y_train_one_hot, batch_size=2048, epochs=100, validation_data=(x_test, y_test_one_hot),
                    callbacks=[early_stopping])  # 
            pred += dnn.predict(x_test)  # 7451*65 float
            continue
        elif clf_type == 'RF':
            # clf = RandomForestClassifier(n_estimators=100) 
            clf = RandomForestClassifier(n_estimators=100)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_input', default=5824, type=int)
    ...
    ddi_info = pd.read_csv('./graph/ddi_class_65.csv', dtype=int)
    ddi_adj = ddi_info.values
    label = ddi_info['class']
    label=label.values
    result_all = combine_and_predict(embbedings, ddi_adj, 'DDIMDL', 65, 3407, 5, label)
    print (result_all)
